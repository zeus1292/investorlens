  ...
  return (
    <div className="rounded-xl bg-white border border-surface-200 overflow-hidden shadow-sm">
      <div className="px-4 py-3 border-b border-surface-200">
        <h3 className="text-sm font-medium text-surface-600">Cross-Persona Comparison</h3>
      </div>
            ...
            {[0, 1, 2, 3, 4].map((idx) => (
              <tr key={idx} className="border-b border-surface-100 hover:bg-surface-50">
                ...
                {PERSONA_ORDER.map((pName) => {
                  ...
                  if (!r) return <td key={pName} className="px-3 py-2 text-xs text-surface-400">—</td>;
                  const isActive = pName === activePersona;
                  return (
                    <td key={pName} className="px-3 py-2">
                      <div className={`text-xs ${isActive ? 'text-surface-900 font-medium' : 'text-surface-600'}`}>
                        {r.name}
                      </div>

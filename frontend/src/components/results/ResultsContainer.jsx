...
function PieChartInfo() {
  return (
    <div className="relative group inline-flex items-center gap-1.5 cursor-help select-none">
      ...
      <div className="absolute top-full right-0 mt-2 w-80 bg-surface-900 text-white text-xs rounded-xl p-3.5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-30 shadow-xl">
        {/* Arrow pointing up */}
        <div className="absolute bottom-full right-4 border-4 border-transparent border-b-surface-900" />
        <p className="font-semibold mb-2">Reading the score breakdown</p>
        <ul className="text-surface-300 leading-relaxed space-y-1 list-disc list-inside">
          <li>Click any card to expand its pie chart</li>
          <li>Each <span className="text-white">slice</span> = one scoring factor for this persona</li>
          <li>Bigger slice = higher weight in the ranking formula</li>
          <li>The <span className="text-white">number</span> next to each label is that factor&apos;s contribution out of 100 points</li>
          <li><span className="text-white">Graph Boost</span> is added for direct graph relationships (e.g. COMPETES_WITH strength)</li>
          <li>The <span className="text-white">overall score (0–1)</span> is the weighted sum of all factors</li>
        </ul>
      </div>
    </div>
  );

...
function CompanyProfile({ company, label, personaColor }) {
  ...
  return (
    <div className="rounded-xl bg-surface-800 border border-surface-700 p-4">
      <h3 className="text-white font-semibold mb-2" style={{ color: personaColor }}>
        {label}: {company.name}
      </h3>
      <div className="grid grid-cols-2 gap-2 text-xs">
        {[
          ...
          <div key={label} className="flex justify-between">
            ...
            <span className="text-surface-300 font-mono">{typeof val === 'number' ? val.toFixed(1) : val}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      {compareData?.shared_edges?.length > 0 && (
        <div className="flex flex-wrap gap-2">
          ...
          {compareData.shared_edges.map((e, i) => {
            ...
            return (
              <span key={i} className="px-2 py-0.5 rounded text-[10px] font-medium" style={{ backgroundColor: `${color}20`, color }}>
      ...
      {compareData?.shared_themes?.length > 0 && (
        <div className="flex flex-wrap gap-2">
          ...
          {compareData.shared_themes.map((t) => (
            <span key={t} className="px-2 py-0.5 rounded text-[10px] bg-surface-700 text-surface-300">
              {t.replace(/_/g, ' ')}
            </span>
          ))}
        </div>
